...
export default function AnalysisCategoryPage() {
  ...
  if (isLoading) {
    return <AppLoadingState label={ui.analysis.categoryPageLabel} />
  }
    ...
    <div className="space-y-6">
      <div className={`rounded-[32px] p-6 ${theme === 'modern' ? isDark ? 'border border-white/10 bg-white/5 shadow-sm' : 'border border-white/80 bg-white/80 shadow-sm' : 'border border-slate-200 bg-white'}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{ui.analysis.categoryPageTitle}</h1>
            <p className={`mt-2 text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{ui.analysis.categoryPageDesc(activeMonth)}</p>
          </div>
      ...
      <div className={`rounded-[32px] p-6 ${theme === 'modern' ? isDark ? 'border border-white/10 bg-white/5 shadow-sm' : 'border border-white/80 bg-white/80 shadow-sm' : 'border border-slate-200 bg-white'}`}>
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{ui.analysis.categoryAmount}</h2>
            <p className={`mt-1 text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{ui.analysis.totalExpense(Math.round(total).toLocaleString(), data.baseCurrency)}</p>
          </div>
        </div>
